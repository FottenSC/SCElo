...
export function getPlayerAvatarUrl(
  ...
  // If we've already tried this Twitter handle and it failed, return empty string
  ...
  const url =
    `https://unavatar.io/twitter/${username}?size=${size}&fallback=false&format=jpeg&quality=80`;
 ...
 */
export function getPlayerAvatarLqip(
  twitter?: string | null,
  fallbackSeed: string = "default",
): string {
  if (!twitter) {
  ...
  const url =
    `https://unavatar.io/twitter/${username}?size=8&fallback=false&format=jpeg`;
